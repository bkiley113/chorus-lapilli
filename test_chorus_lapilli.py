...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    def test_three_in_a_row(self):
        '''Check if three in a row of either X or O wins the game. Ensures no more clicks 
        after a win'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()
        self.assertTileIs(tiles[0], self.SYMBOL_X)
    ...
    def test_three_only(self):
        '''Check to make sure that no pieces are added after three X's and O's each'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()
        self.assertTileIs(tiles[0], self.SYMBOL_X)
        tiles[1].click()
    ...
    def test_vacate_center(self):
        '''Check to make sure that after three moves each, the player must 
        move their pieces to adjacent squares. Does so by vacating the center square per
        the rules'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()
        self.assertTileIs(tiles[0], self.SYMBOL_X)
